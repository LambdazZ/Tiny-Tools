...
# 获取视频BV号
def get_video_bv_numbers(keyword: str, bv_number_count: int) -> set:
    """
    ...
    try:
        ...
        while len(res) < bv_number_count:
            ...
            driver.get(url)
            driver.refresh()
            # 等待页面完全加载完全或时间太长无法加载转到下一页
            time.sleep(10)

            element_content = driver.page_source
            element_content = element_content[
                              element_content.find(r'video-list row'): element_content.find(r'to_hide_xs')]
            pattern = r'BV(.*?)/"'
...
# 获得cid
async def get_video_cid(session, bvid: str) -> int:
    """
    异步获取指定 BV 号视频的 CID。
...
# 获得弹幕
async def get_dm(session, cid: str) -> None:
    """
    异步获取指定 CID 的弹幕信息。
...
async def process_bvid(session, bvid) -> None:
    """
    异步处理指定 BV 号的视频。
...
# 导入EXCEL
def export_to_excel(sorted_dict: dict, excel_name: str, rows: int = -1) -> str:
    """
